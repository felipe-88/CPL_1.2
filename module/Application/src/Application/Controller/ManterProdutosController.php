...
class ManterProdutosController extends AbstractActionController {
    ...
    public function adicionarAction() {
        ...
        $descricaoCompletaTxt = $request->getPost('descricaoCompletaTxt');
        $tipoSlct = $request->getPost('tipoSlct');
        $unidadeSlct = $request->getPost('unidadeSlct');

        $dadosFiltrados = new ProdutoFilter($descricaoCompletaTxt,$descricaoTxt, $tipoSlct, $unidadeSlct, $tipoProdutoDAO, $unidadeDAO);
        ...
        $parametros->set('descricaoCompleta', $dadosFiltrados->getValue('descricaoCompletaTxt'));
    ...
    public function buscarAction() {
        $request = $this->getRequest();
        //usando htmlentities() para converter caracteres digitados no campo de
        //pesquisa para caracteres HTML válidos
        $busca = htmlentities($this->params()->fromQuery('busca'));
        $tipo = $this->params()->fromQuery('tipoSlct');
        if ((($busca == null) && ($tipo == null)) || (!$request->isGet())) {
            $this->redirect()->toRoute('produtos');
            return;
        }

        $parametrosBusca = new ArrayCollection();
        $parametrosBusca->set('tipo', $tipo);
    ...
    public function editarAction() {
        ...
        if ($produto == NULL) {
            $this->flashMessenger()->addMessage("Produto não encontrado.");
            $this->redirect()->toRoute('produtos');
            return;
        }
