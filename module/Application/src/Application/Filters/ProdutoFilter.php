...
class ProdutoFilter extends InputFilter{
    ...
    public function __construct($descricaoCompletaTxt ,$descricaoTxt, $tipoSlct, $unidadeSlct,
            DAOInterface $tipoProdutoDAO, DAOInterface $unidadeDAO) {
        
        $this->descricaoCompletaTxt = $descricaoCompletaTxt;
        ...
        $this->add($this->getDescricaoCompletaTxtInput());
    ...
    public function getDescricaoTxtInput(){
        ...
        $descricaoNotEmpty->setMessage('O campo descrição resumida é obrigatório e não pode ser vazio.');
    ...
    public function getDescricaoCompletaTxtInput(){
        $descricaoCompletaInputFilter = new Input('descricaoCompletaTxt');
        
        $descricaoCompletaStringLength = new Validator\StringLength(array('min' => 3));
        
        $descricaoCompletaStringLength->setMessages(array(
            Validator\StringLength::TOO_SHORT =>
                'A descrição é muito curta, o valor mínimo é de %min% caracteres.'
        ));
        
        $descricaoCompletaInputFilter->getValidatorChain()
                ->attach($descricaoCompletaStringLength);
        $descricaoCompletaInputFilter->getFilterChain()
                ->attach(new Filter\HtmlEntities())
                ->attach(new Filter\StringTrim())
                ->attach(new Filter\StripTags());
        
        return $descricaoCompletaInputFilter;
    }
    ...
    public function preencherDados(){
        $this->setData(array(
            'descricaoCompletaTxt' => $this->descricaoCompletaTxt,
